...
def test_profile_by_riot_id_success(monkeypatch) -> None:
    ...
    async def fake_get_league_entries_by_puuid(self, encrypted_puuid: str):
        return [
            {
    ...
    monkeypatch.setattr(RiotClient, "get_league_entries_by_puuid", fake_get_league_entries_by_puuid)
...
def test_profile_by_riot_id_falls_back_to_summoner_rank_lookup(monkeypatch) -> None:
    monkeypatch.setenv("RIOT_API_KEY", "test-key")
    monkeypatch.setenv("RIOT_REGION_ROUTING", "europe")
    monkeypatch.setenv("RIOT_PLATFORM_ROUTING", "euw1")
    ...
    assert response.status_code == 200
    payload = response.json()
    assert payload["ranked"][0]["tier"] == "PLATINUM"
    assert payload["ranked"][0]["queue_type"] == "RANKED_SOLO_5x5"
...
def test_player_summary_success(monkeypatch) -> None:
    ...
    client = TestClient(app)
    response = client.get("/v1/players/by-riot-id/ash%20timmy/ASH/summary?count=2")
...
def test_player_trend_success(monkeypatch) -> None:
    ...
    client = TestClient(app)
    response = client.get("/v1/players/by-riot-id/ash%20timmy/ASH/performance-trend?count=5&recent_window=3")
